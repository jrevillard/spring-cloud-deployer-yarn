...
/**
 * Default implementation of {@link YarnCloudAppService} which talks to rest api's exposed by
 * specific yarn controlling container clusters.
 *
...
public class DefaultYarnCloudAppService implements YarnCloudAppService, InitializingBean {
  ...
  private static final ThreadPrintStream THREADED_SYSTEM_OUT = new ThreadPrintStream();
  private static final ThreadPrintStream THREADED_SYSTEM_ERR = new ThreadPrintStream();
  ...
  public String submitApplication(String appVersion, CloudAppType cloudAppType,
    ...
    return getApp(appVersion, deployerVersion, cloudAppType, contextRunArgs)
        .submitApplication(appVersion);
  }
  ...
  public String getApplicationLogs(String yarnApplicationId, CloudAppType cloudAppType) {
    YarnCloudAppServiceApplication app = getApp(null, null, cloudAppType);
    YarnConfiguration yarnConfiguration = app.getContext()
        .getBean(YarnSystemConstants.DEFAULT_ID_CONFIGURATION, YarnConfiguration.class);

    String[] args = {"-applicationId", yarnApplicationId};
    ...
    try (
      ...
      THREADED_SYSTEM_OUT.setThreadStream(ps);
      THREADED_SYSTEM_ERR.setThreadStream(ps);
      System.setOut(ps);
      System.setErr(ps);
      logDumper.run(args);
      return baos.toString(StandardCharsets.UTF_8.name());
    } catch (Exception e) {
      logger.warn("Error when collecting Yarn Application logs ({})", yarnApplicationId, e);
      throw new RuntimeException(
          "Error when collecting Yarn Application logs (" + yarnApplicationId + ")", e);
    } finally {
      THREADED_SYSTEM_OUT.close();
      THREADED_SYSTEM_ERR.close();
    }
  }
  ...
  public void createCluster(String yarnApplicationId, String clusterId, int count, String artifact,
    ...
    for (Map.Entry<String, String> entry : definitionParameters.entrySet()) {
      ...
      if (value.startsWith("\"") && value.endsWith("\"")) {
        ...
        extraProperties.put("containerArg" + i++,
            entry.getKey() + "=\\" + value.substring(0, value.length() - 1) + "\\\"");
      } else {
        // prevent ${xxx.ddd} bad substitution with bash
        value = value.replace("$", "\\\\\\$");
    ...
    getApp(null, null, CloudAppType.STREAM).createCluster(
        ApplicationId.fromString(yarnApplicationId), clusterId, "module-template", "default", count,
        null, null, null, extraProperties);
  }

  @Override
  public void startCluster(String yarnApplicationId, String clusterId) {
    getApp(null, null, CloudAppType.STREAM)
        .startCluster(ApplicationId.fromString(yarnApplicationId), clusterId);
  }

  @Override
  public void stopCluster(String yarnApplicationId, String clusterId) {
    getApp(null, null, CloudAppType.STREAM).stopCluster(ApplicationId.fromString(yarnApplicationId),
        clusterId);
  }

  @Override
  public Map<String, StreamClustersInfoReportData> getClustersStates(String yarnApplicationId) {
    HashMap<String, StreamClustersInfoReportData> states = new HashMap<>();
    Collection<CloudAppInstanceInfo> submittedApplications =
        getApp(null, null, CloudAppType.STREAM).getSubmittedApplications(yarnApplicationId);
    for (CloudAppInstanceInfo instanceInfo : submittedApplications) {
      if (instanceInfo.getName().startsWith("scdstream:")
          && instanceInfo.getState().equals("RUNNING")) {
  ...
  public Collection<String> getClusters(String yarnApplicationId) {
    return getApp(null, null, CloudAppType.STREAM)
        .getClustersInfo(ApplicationId.fromString(yarnApplicationId));
  }

  @Override
  public void destroyCluster(String yarnApplicationId, String clusterId) {
    getApp(null, null, CloudAppType.STREAM)
        .destroyCluster(ApplicationId.fromString(yarnApplicationId), clusterId);
  }

  private Map<String, StreamClustersInfoReportData> getInstanceClustersStates(
      String yarnApplicationId, String clusterId) {
    HashMap<String, StreamClustersInfoReportData> states = new HashMap<>();
    List<StreamClustersInfoReportData> clusterInfo = getApp(null, null, CloudAppType.STREAM)
        .getClusterInfo(ApplicationId.fromString(yarnApplicationId), clusterId);
  ...
  private synchronized YarnCloudAppServiceApplication getApp(String appVersion,
      String dataflowVersion, CloudAppType cloudAppType) {
    return getApp(appVersion, dataflowVersion, cloudAppType, null);
  }

  private synchronized YarnCloudAppServiceApplication getApp(String appVersion,
      String dataflowVersion, CloudAppType cloudAppType, List<String> contextRunArgs) {
    contextRunArgs = processContextRunArgs(contextRunArgs);
    String cacheKey =
        cloudAppType + appVersion + StringUtils.collectionToCommaDelimitedString(contextRunArgs);
    ...
    if (app == null) {
      ...
      logger.info("Bootsrapping YarnCloudAppServiceApplication with {}",
          cloudAppType.toString().toLowerCase());
      ...
      app =
          new YarnCloudAppServiceApplication(appVersion, dataflowVersion, "application.properties",
              configFileProperties, runArgs.toArray(new String[0]), initializers);
      try {
